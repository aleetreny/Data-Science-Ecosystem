---
title: 'Student Performance Analysis: Exploratory and Predictive Modeling'
author: "ALEJANDRO TRENY ORTEGA"
date: "`r Sys.Date()`"
output:
  pdf_document:
    toc: false
    number_sections: true
    fig_caption: true
    keep_tex: false
    latex_engine: xelatex
geometry: margin=1.5cm
fontsize: 10pt
classoption: twocolumn
header-includes:
  - \usepackage{float}
  - \raggedbottom
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
  echo = FALSE,
  warning = FALSE,
  message = FALSE,
  fig.align = "center",
  fig.width = 3.5,
  fig.height = 3,
  fig.pos = "H",
  out.width = "100%"
)

library(ggplot2)
library(dplyr)
library(rpart)
library(rpart.plot)
library(caret)
library(knitr)
```

```{r load-data, include=FALSE}
student_data <- read.csv("student_data.csv", sep = ";", header = TRUE)
```

# PART A: Exploratory Data Analysis

This report presents a comprehensive exploratory analysis of the "Student Performance" dataset from the UCI Machine Learning Repository. The dataset contains information on 395 students from Portuguese secondary schools, encompassing demographic characteristics, family background, academic history, and final performance metrics. Our analysis focuses on identifying key associations with student academic achievement through well-structured research questions and appropriate statistical visualization techniques.

## Research Question 1: Parental Education and Grades

**Question:** How is maximum parental education associated with mean final grade (G3)?

**Methodology:** We created a composite variable representing the higher educational attainment between mother (Medu) and father (Fedu), as a descriptive summary of the two parental education measurements. A bar chart with error bars (standard error of the mean) visualizes the relationship between parental education levels and student performance.

```{r q1-prep, include=FALSE}
student_data$max_parent_edu <- pmax(student_data$Medu, student_data$Fedu)

parent_edu_summary <- student_data %>%
  group_by(max_parent_edu) %>%
  summarise(mean_G3 = mean(G3), count = n(), sd_G3 = sd(G3), .groups = "drop")
```

```{r q1-viz, fig.cap="Maximum Parental Education and Student Final Grade"}
ggplot(parent_edu_summary, aes(x = factor(max_parent_edu), y = mean_G3,
                               fill = factor(max_parent_edu))) +
  geom_col(color = "black", width = 0.65) +
  geom_errorbar(aes(ymin = mean_G3 - sd_G3/sqrt(count),
                    ymax = mean_G3 + sd_G3/sqrt(count)), width = 0.15) +
  geom_text(aes(y = mean_G3 + sd_G3/sqrt(count) + 0.25,
                label = sprintf("%.2f", mean_G3)), vjust = 0, size = 2.5) +
  labs(x = "Maximum Parental Education Level", y = "Mean Final Grade (G3)",
       title = "Parental Education and Grades") +
  scale_x_discrete(labels = c("0"="None", "1"="4th grade", "2"="5-9th grade", "3"="Secondary", "4"="Higher education")) +
  scale_fill_brewer(palette = "Blues") +
  theme_minimal(base_size = 8) +
  theme(legend.position = "none", axis.title = element_text(size = 8),
        plot.title = element_text(size = 9, face = "bold"),
        axis.text.x = element_text(angle = 45, hjust = 1, size = 7))
```

**Interpretation:** Mean final grade increases across these parental-education groups, from 8.51 to 11.57. The bars show mean plus/minus one standard error, not confidence intervals or a hypothesis test. These unadjusted observational associations do not establish a causal mechanism.

## Research Question 2: Grade Distribution Analysis

**Question:** What is the distribution of final grades (G3) relative to the passing threshold (10)?

**Methodology:** We transformed the final grades by subtracting the passing threshold (10 points) to create a "distance-to-pass" variable. This approach provides immediate insight into the proportion of students above, at, or below the critical performance benchmark. A histogram with color-coded regions effectively communicates the distribution patterns.

```{r q2-prep, include=FALSE}
df_di <- transform(student_data, pass_margin = G3 - 10)

pass_pct <- round(100*mean(df_di$G3 >= 10), 1)

below_pct <- round(100*mean(df_di$G3 < 10), 1)
```

```{r q2-viz, fig.cap="Distribution of Final Grades Relative to Passing Threshold"}
ggplot(df_di, aes(pass_margin)) +
  geom_histogram(aes(fill = after_stat(ifelse(x < 0, "Below pass",
                                               ifelse(x == 0, "Exactly 10", "Above pass")))),
                 binwidth = 1, boundary = -0.5, color = "white", alpha = 0.95) +
  scale_fill_manual(
    limits = c("Below pass","Exactly 10","Above pass"),
    values = c("Below pass"="#e15759","Exactly 10"="#9e9e9e","Above pass"="#4e79a7"),
    name = NULL
  ) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black", size = 0.5) +
  labs(x = "Distance to Pass (G3 - 10)", y = "Count",
       title = "Distance to Pass Threshold") +
  theme_minimal(base_size = 8) +
  theme(legend.position = "top", axis.title = element_text(size = 8),
        plot.title = element_text(size = 9, face = "bold"))
```

**Interpretation:** `r pass_pct`% of students pass and `r below_pct`% fail. A recorded zero does not by itself identify dropout or exam absence. The figure describes outcomes; it does not estimate an intervention effect.

## Research Question 3: Educational Aspirations and Grades

**Question:** How does a student's aspiration for higher education relate to their final grade (G3) distribution?

**Methodology:** We employed a combination visualization (violin plot overlaid with boxplot) to examine both the distribution shape and central tendencies of grades by educational aspiration. This approach reveals not only differences in medians but also the entire distribution characteristics, including potential multimodality or skewness patterns.

```{r q3-prep, include=FALSE}
df_hi <- transform(student_data, higher = factor(higher, c("no","yes"), c("No","Yes")))

aspir_counts <- df_hi %>% group_by(higher) %>%
  summarise(n_total = n(), n_pass = sum(G3 >= 10), pass_rate = round(100*mean(G3 >= 10)))
```

```{r q3-viz, fig.cap="Final Grades by Higher Education Aspiration"}
ggplot(df_hi, aes(higher, G3, fill = higher)) +
  geom_violin(trim = TRUE, alpha = 0.35, color = NA) +
  geom_boxplot(width = 0.15, alpha = 1, outlier.size = 0.8) +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 2,
               fill = "white", color = "black") +
  geom_hline(yintercept = 10, linetype = "dashed", color = "gray") +
  scale_fill_manual(values = c("No"="#ee6d6a", "Yes"="#9dbbe0"), guide = "none") +
  labs(x = "Wants Higher Education", y = "Final Grade (G3)",
       title = "Aspirations and Grades") +
  theme_minimal(base_size = 8) +
  theme(axis.title = element_text(size = 8), plot.title = element_text(size = 9, face = "bold"))
```

**Interpretation:** Pass rates are 68.8% among students reporting higher-education aspirations and 35.0% among those not reporting them. The groups differ in other observed and unobserved characteristics, so the association cannot establish whether aspirations change grades.

## Research Question 4: Academic History Associations

**Question:** How do the observed G2 and G3 values vary across groups with and without past academic failures?

**Methodology:** A scatter plot colors second-period (G2) and final grades (G3) by failure history and overlays a pooled regression line. It describes the sample relationship without testing differences in slopes between groups.

```{r q4-viz, fig.cap="Relationship between G2 and G3 by Academic Failure History"}
student_data %>%
  mutate(FailGroup = factor(ifelse(failures > 0, "1+ Failures", "0 Failures"),
                            levels = c("0 Failures", "1+ Failures"))) %>%
  ggplot(aes(x = G2, y = G3, color = FailGroup)) +
  geom_point(alpha = 0.7, size = 1.5) +
  geom_smooth(method = "lm", se = FALSE, color = "black", linewidth = 0.7, aes(group = 1)) +
  scale_color_manual(values = c("0 Failures" = "#4daf4a", "1+ Failures" = "#e41a1c")) +
  labs(x = "Second Period Grade (G2)", y = "Final Grade (G3)",
       title = "G2-G3 Relationship",
       color = "Academic History") +
  theme_minimal(base_size = 8) +
  theme(legend.position = "bottom", legend.text = element_text(size = 7),
        axis.title = element_text(size = 8), plot.title = element_text(size = 9, face = "bold"))
```

**Interpretation:** The Pearson correlation between G2 and G3 is `r round(cor(student_data$G2, student_data$G3), 3)`. The black regression line is pooled across failure-history groups. It does not test a difference in slopes or establish the effect of past failure.

## Research Question 5: Rank Correlation Analysis

**Question:** Which numerical/ordinal factors have the strongest correlation with final grade (G3)?

**Methodology:** We calculated Spearman rank correlations between the selected numeric/ordinal variables and G3, presenting results via a lollipop plot. Rank correlations describe marginal monotonic associations; they do not measure adjusted predictive importance.

```{r q5-prep, include=FALSE}
numeric_data <- student_data %>%
  select(age, Medu, Fedu, traveltime, studytime, failures, famrel, freetime,
         goout, Dalc, Walc, health, absences, G1, G2, G3)

cor_matrix <- cor(numeric_data, method = "spearman")

cor_g3_vec <- cor_matrix[, "G3"]

cor_g3 <- data.frame(
  variable = names(cor_g3_vec),
  correlation = as.numeric(cor_g3_vec),
  stringsAsFactors = FALSE
) %>%
  filter(variable != "G3") %>%
  mutate(type = ifelse(correlation > 0, "Positive", "Negative")) %>%
  arrange(correlation)
```

```{r q5-viz, fig.cap="Spearman Correlation of Factors with Final Grade"}
ggplot(cor_g3, aes(x = correlation, y = reorder(variable, correlation), color = type)) +
  geom_segment(aes(x = 0, yend = variable, xend = correlation), linewidth = 0.8) +
  geom_point(size = 2.5) +
  scale_color_manual(values = c("Negative" = "#e63946", "Positive" = "#457b9d")) +
  labs(x = "Spearman rho", y = NULL,
       title = "Correlations with G3") +
  theme_minimal(base_size = 7) +
  theme(legend.position = "none", panel.grid.major.y = element_blank(),
        axis.text = element_text(size = 6))
```

**Interpretation:** The strongest Spearman correlations with G3 are G2 (`r round(cor_matrix["G2", "G3"], 3)`) and G1 (`r round(cor_matrix["G1", "G3"], 3)`). Past failures correlate negatively (`r round(cor_matrix["failures", "G3"], 3)`), whereas absences are close to uncorrelated (`r round(cor_matrix["absences", "G3"], 3)`). These marginal associations are not causal effects or adjusted predictive importances.

\onecolumn
\newpage

# PART B: Predictive Decision Tree Analysis

## Objective and Methodology

This section evaluates a decision tree model to predict whether students achieve satisfactory performance (G3 >= 10) using demographic, social, and academic variables while excluding prior grades (G1, G2) to examine prediction without earlier grade measurements.

## Model Development Process

**Data Preparation:** We created a binary target variable classifying students as passing (G3 >= 10) or failing (G3 < 10). The dataset was split into 70% training (277 observations) and 30% testing (118 observations) using stratified sampling to maintain class proportions.

**Model Training:** We employed 10-fold cross-validation with ROC optimization to tune the complexity parameter (cp), testing 34 different values from 0.001 to 0.1. Cross-validation selects among the specified candidates; it does not guarantee prevention of overfitting.

## Optimized Decision Tree Structure

```{r tree-prep, include=FALSE}
student_data$high_performance <- factor(ifelse(student_data$G3 >= 10, "Yes", "No"),
                                        levels = c("Yes", "No"))

set.seed(123)

train_idx <- createDataPartition(student_data$high_performance, p = 0.7, list = FALSE)

train_dat <- student_data[train_idx, ]

test_dat <- student_data[-train_idx, ]

predictors <- setdiff(names(student_data),
                      c("G1", "G2", "G3", "high_performance", "max_parent_edu"))

formula <- as.formula(paste("high_performance ~", paste(predictors, collapse = " + ")))

train_ctrl <- trainControl(method = "cv", number = 10, summaryFunction = twoClassSummary,
                           classProbs = TRUE, savePredictions = "final")

tune_grd <- expand.grid(cp = seq(0.001, 0.1, by = 0.003))

set.seed(123)

tree_model <- train(formula, data = train_dat, method = "rpart", trControl = train_ctrl,
                    metric = "ROC", tuneGrid = tune_grd)

final_tree <- tree_model$finalModel

preds <- predict(tree_model, test_dat)

conf <- confusionMatrix(preds, test_dat$high_performance, positive = "Yes")
majority_class <- names(which.max(table(train_dat$high_performance)))
baseline_accuracy <- mean(test_dat$high_performance == majority_class)
```

```{r tree-plot, fig.width=9.5, fig.height=7, fig.cap="Optimized Decision Tree for Predicting Passing"}
rpart.plot(final_tree, main = "Decision Tree: Predicting Passing (G3 >= 10)",
           extra = 104, box.palette = "RdYlGn", branch.lty = 3,
           shadow.col = "gray85", nn = TRUE, split.yshift = -1, split.yspace = 2)
```

**Tree interpretation:** Read the fitted splits and leaf probabilities from the diagram. They describe associations in this training sample; leaf probabilities are not calibrated individual risk estimates, and split order is not a causal ranking.

## Model Performance Evaluation

**Performance Metrics on Test Set:**

- **Accuracy:** `r round(conf$overall["Accuracy"], 4)`

- **Kappa:** `r round(conf$overall["Kappa"], 4)` (agreement beyond chance)

- **Sensitivity (Recall):** `r round(conf$byClass["Sensitivity"], 4)`

- **Specificity:** `r round(conf$byClass["Specificity"], 4)`

- **Precision (Positive Predictive Value):** `r round(conf$byClass["Pos Pred Value"], 4)`

- **F1-Score:** `r round(2*(conf$byClass["Sensitivity"]*conf$byClass["Pos Pred Value"])/(conf$byClass["Sensitivity"]+conf$byClass["Pos Pred Value"]), 4)` (balance of precision and recall)

**Performance Analysis:** Test accuracy is `r round(conf$overall["Accuracy"], 4)`, compared with `r round(baseline_accuracy, 4)` for always predicting the training-majority class. Specificity is recall for failing students: `r round(conf$byClass["Specificity"], 4)`. These results do not support describing this model as an effective early-warning system. Accuracy alone also does not measure probability ranking or calibration.

## Conclusions and limitations

This analysis describes one sample of 395 students and evaluates a tree on 118 held-out observations. Cross-validation selects the complexity parameter using the training partition only. The outcome means passing, not exceptional academic performance.

The descriptive associations do not identify causes or demonstrate that an intervention would improve outcomes. Excluding G1 and G2 is insufficient to establish an early-warning use case: the time at which remaining variables, including accumulated absences, become available would also need verification.

The held-out results must be compared with the majority-class baseline and the poor recall for failing students. Independent schools or cohorts, probability calibration, uncertainty estimates and an assessment of intervention costs would be required before considering educational use.

Dataset: [UCI Student Performance](https://archive.ics.uci.edu/dataset/320/student+performance), mathematics subset, Cortez and Silva (2008).
